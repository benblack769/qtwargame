...
void SimpleCompPlayer::DoAITurn(vector<Player *> & InPlayers){
    ...
    AttackEnemies();
    ChangeTurn();//p
    ChangeOverallTurn();//todo: this leads to infinite recursion, and will crash program eventually
    ShowOnScreen();
}
...
void SimpleCompPlayer::GetThreat(){
    ...
    for(EPlayer * Play:Enemies){
        for(Troop * T: Play->Troops){
            ...
            for (Point AttackP : SquareIterate(T->GetSpot(), ThisMoveAttackRange)){
...
#ifdef Debug_Macro_Move
void SimpleCompPlayer::ShowOnScreen(){
    Player::ShowOnScreen();
}
#endif
...
bool SimpleCompPlayer::CanMoveToSafety(Troop * T){
    ...
    FastSquareIter(T->GetSpot(),T->Range,[&](Point P){
...
void SimpleCompPlayer::AttackEnemies(){
    ...
    for (auto & AttackPair:ClaimedAttackSpots){
        ...
        Point P1 = T->GetSpot(),P2 = AttackPair.second;
    ...
    for (Troop * T : FailedAttacks){
        PointVal BestP = GetBestAttackPoint(T->GetSpot(), T);
