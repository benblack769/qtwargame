...
void AttackLocation(Point Loc, Troop * AttackingTroop){//decleration is in globals.h
    ...
    int AttackPlayerNum = PlayerOcc[AttackingTroop->GetSpot()];
...
void InitiateGame(DebugInter *deb){
    PlayerTurn = 0;
    srand(clock());//acts as a random enough seed
    Players.resize(2);
    Players[0] = new RealPlayer;
    Players[0]->PlayerNum = 0;
    Players[0]->TeamNum = 0;
    ComputerPlayer * compplay = new ComputerPlayer;
    Players[1] = compplay;
    Players[1]->PlayerNum = 1;
    Players[1]->TeamNum = 1;
    ResetGame();
#ifdef Debug_Macro_Move
    deb->compplay = compplay;
#endif
}
...
void ChangeMiniTurn(){
    if(GetCurrentPlayer()->ChangeTurn()){
        ChangeOverallTurn();
    }
}
